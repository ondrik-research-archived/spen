...
(assert 
	...
	(iff (subset (bag key) M0) (subset (bag key) M3) ) (> d1 key) (distinct parent1 nil) 
	(= is_even 0) (> d2 key) (= cur2 Y) (= M5 (ite (subset (bag key) M0) M3 (bagunion M3 (bag key)) ) ) 
	(= N5 N3) (= C5 C3) 
	)
)
...
(assert (not 
	...
	(= M1  (ite (subset (bag key) M0) M0 (bagunion M0 (bag key))) ) (= C1 1)
	(iff (subset (bag key) M0) (subset (bag key) M6) ) (> d2 key) (distinct parent2 nil) (= is_even 0)
	)
))
