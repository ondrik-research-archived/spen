...
;; VC21: cusnode |-> ((left,X),(right,unc),(data,d1),(color,0)) * rbt(unc,M2,N2,C2) * pa |-> ((left,gra),(right,cusnode),(data,d2),(color,1)) * rbt(X,M3,N3,C3) * gra 
;; |-> ((left,U1),(right,V1),(data,d3),(color,0)) * rbt(V1,M7,N7,1) * U1|-> ((left,U2),(right,V2),(data,d4),(color,1)) * rbt(V2,M8,N8,C8) * rbrbthole
;; (U2,x,M9,N9,0,M5,N5,0) * x |-> ((left,nil),(right,nil),(data,key),(color,0)) &  
;; M9 < d4 < M8 & N9 = N8 & M9 cup {d5} cup M8 < d3 < M7 & N8+1 = N7 & M4 = M9 cup {d4} cup M8 cup {d3} cup M7 & N4 =N7
;; & M5 = {key} & N5 = 1 & M4 < d2 < M3 & N4 = N3 & 0 <= c2 <= 1 & (c2 =0 => C3 =1) & {d2} cup M4 cup M3 < d1 < M2 & n2 = ite(c2 =0, N3, N3+1) & n2=N2 & 0 <=c1 <= 1 & 
;; (c1=0 => c2 = 1 & C2=1) & M0 cup {key} = {d2} cup M4 cup M3 cup {d1} cup M2 & ! key in M0 & cusparent = nil & c2 = 0 & C2 = 1 & root = pa & ret = root & M6 = {d2} 
;; cup M4 cup M3 cup {d1} cup M2 & N6 = N3+1 & C6=1
;; |-
...
(assert 
	...
	(tobool 
	(ssep 
		(pto cusnode (sref (ref left X) (ref right unc) (ref data d1) (ref color 0)))
		(index alpha1 (rbt unc M2 N2 C2))
		(pto pa (sref (ref left gra) (ref right cusnode) (ref data d2) (ref color 1)))
		(index alpha2 (rbt X M3 N3 C3))
		(pto gra (sref (ref left U1) (ref right V1) (ref data d3) (ref color 0)))
		(index alpha3 (rbt V1 M7 N7 1))
		(pto U1 (sref (ref left U2) (ref right V2) (ref data d4) (ref color 1)))
		(index alpha4 (rbt V2 M8 N8 C8))
		(index alpha5 (rbrbthole U2 x M9 N9 0 M5 N5 0))
		(pto x (sref (ref left nil) (ref right nil) (ref data key) (ref color 0)))  
	))
	(< M9 (bag d4)) (< (bag d4) M8) (= N9 N8)
	(< (bagunion M9 (bag d5) M8 (bag d3)) (< (bag d3) M7)
	(= (+ N8 1) N7) (= M4  (bagunion M9 (bag d4) M8 (bag d3) M7) )
	(= N4 N7) (= M5 (bag key)) (= N5 1) (< M4 (bag d2)) (< (bag d2) M3) (= N4 N3)
	(<= 0 c2) (<= c2 1) (=> (= c2 0) (= C3 1)) (< (bagunion (bag d2) M4 M3) (bag d1)) (< (bag d1) M2) 
	...
	(= cusparent nil) (= c2 0) (= C2 1) (= root pa) (= ret root) (= M6 (bagunion (bag d2) M4 M3 (bag d1) M2) )
	(= N6 (+ N3 1)) (= C6 1)
	)
)
...
(assert (not 
	(and 
	(tobool 
		(index alpha6 (rbt ret M6 N6 C6))
	)
	(= M6 (bagunion M0 (bag key)) ) (= M0 (bagminus M0 (bag key)) )
	)
))
