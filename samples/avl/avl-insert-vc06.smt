...
;; VC06: root |-> ((left,X),(right,Y),(data,d1),(balance,b1)) * bavlhole(X, parent1, M1, H1, M2, H2) * 
;; parent1|->((left,cur1), (right, Z), (data, d2), (balance, b2)) * cur1|->((left,V), (right, U), (data, d3), (balance, b3)) * 
;; avl(V, M7, H7) * avl(U, M8, H8) * avl(Z, M5, H5) * avl(Y, M3, H3) & M4 = {d3} cup M7 cup M8 & M7 < d3 < M8 & 
;; ite(H8 > H7, H4 = H8+1, H4 = H7+1) & b3 = H8 - H7 & -1 <= b3 <= 1 & M4 < d2 < M5 & b2 = H5 - H4 & -1 <= b2 <= 1 & 
...
;; key in M0 <=> key in M4 & ! parent1 = nil & unbparent1 = nil & unbalance1 = root & d1 > key & d2 > key & b2 =0 & 
;; ! cur1 =nil & b3 = 0 & parent2 = cur1 & d3 > key & cur2 = V & ite(key in M4, M6 = M4, M6 = M4 cup {key}) & H6 = H4 & 
;; unbparent2 = unbparent1 & unbalance2 = unbalance1 |-
;; root |-> ((left,X),(right,Y),(data,d1),(balance,b1)) * bavlhole(X, parent2, M1, H1, M6, H6) * 
;; parent2|->((left,cur2), (right, U), (data, d3), (balance, b3)) * avl(cur2, M7, H7) * avl(U, M8, H8) * avl(Y, M3, H3) & 
;; M7 < d3 < M8 & b3 = H8 - H7 & -1 <= b3 <= 1 & ite(key in M7, M6 = {d3} cup M7 cup M8, M6 = {d3} cup M7 cup M8 cup {key}) & 
;; ite(H8>H7, H6 = H8+1, H6 = H7+1) & ite(key in M0, M0 = {d1} cup M1 cup M3, M0 = ({d1} cup M1 cup M3) \ {key}) & M1 < d1 < M3 & 
;; b1 = H3 - H1 & -1<= b1 <= 1 & key in M0 <=> key in M7 & ! parent2 = nil & unbparent2 = nil & unbalance2 = root & d1 > key & 
;; d3 > key & b3 = 0 
...
(assert 
	...
	(= M4 (bagunion (key d3) M7 M8) ) (< M7 (bag d3)) (< (bag d3) M8)
	(ite (> H8 H7) (= H4  (+ H8 1)) (= H4  (+ H7 1)) ) 
	(= b3  (- H8 H7)) (<= -1 b3) (<= b3 1) (< M4 (bag d2)) (< (bag d2) M5) (= b2 (- H5 H4))
	(<= -1 b2) (<= b2 1)
	(ite (subset (bag key) M4) (= M2 (bagunion (bag d2) M4 M5) ) (= M2 (bagunion (bag d2) M4 M5 (bag key)) ) ) 
	(ite (> H5 H4) (= H2  (+ H5 1)) (= H2  (+ H4 1)) )
	(ite (subset (bag key) M0) (= M0  (bagunion (key d1) M1 M3)) (= M0 (bagminus (bagunion (key d1) M1 M3) (bag key) ) ) ) 
	(< M1 (bag d1)) (< (bag d1) M3) (= b1  (- H3  H1))
	(<= -1 b1) (<= b1 1)
	(iff (subset (bag key) M0) (subset (bag key) M4) ) (distinct parent1 nil) (= unbparent1 nil) (= unbalance1 root)
	(> d1 key) (> d2 key) (= b2 0)
	(distinct cur1 nil) (= b3 0) (= parent2 cur1)  (> d3 key)  (= cur2 V) 
	(ite (subset (bag key) M4) (= M6 M4) (= M6 (bagunion M4  (bag key)) ) ) 
	(= H6 H4)	(= unbparent2 unbparent1) (= unbalance2 unbalance1)
	)
)
...
(assert (not 
	...
	(ssep 
		(pto root (sref (ref left X) (ref right Y) (ref data d1) (ref balance b1)))
		(index alpha1 (bavlhole X parent2 M1 H1 M6 H6) )
		(pto parent2 (sref (ref left cur2) (ref right U) (ref data d3) (ref balance b3)))
		(index alpha6 (avl cur2 M7 H7) 
		(index alpha7 (avl U M8 H8))
		(index alpha8 (avl Y M3 H3))
	))
	(< M7 (bag d3)) (< (bag d3) M8)
	(= b3  (- H8 H7)) (<= -1 b3) (<= b3 1)
	...
	(< M1 (bag d1) (< (bag d1) M3)
	(= b1 (- H3 H1)) (<= -1 b1) (<= b1 1) (iff (subset (bag key) M0) (subset (bag key) M7) ) 
	(distinct parent2 nil) (= unbparent2 nil) (= unbalance2 root) (> d1 key) (> d3 key) (= b3 0) 
)
))
