...
;; VC19: avlhole(root, unbparent, M1,H1,M2,H2) * unbparent |-> ((left,lft),(right,X),(data,d1),(balance,b1)) * 
;; unbalance |-> ((left,V),(right,Z),(data,d2),(balance,0)) * lft |-> ((left,U),(right,unbalance),(data,d4),(balance,0)) * 
;; ubavlhole(U,parent,M9,H10+1,M7,2) * avl(V,M10,H10) * parent |-> ((left,x),(right,nil),(data,d3),(balance,-1)) * 
;; x |-> ((left,nil),(right,nil),(data,key), (balance,0)) * avl(Z, M5,H10) * avl(X, M3, H3) & M4 = {d4} cup M9 cup M10 & 
;; M9 < d4 < M10 & !b4 = 0 & M7 = {d3} cup {key} & M4 < d2 < M5 & M2 = {d1} cup {d2} cup M4 cup M5 cup M3 & 
;; {d2} cup M4 cup M5 < d1 < M3 & & b1 = H3 - (H10+2) & -1<= b1 <= 1  &  M1 = M0 cup {key} & ! key in M0 & ! parent = nil & 
;; ! unbparent = nil & d1> key & d2 > key & d3 > key & cur = x & ret = root
;; |-
...
(assert 
	...
	(ssep 
		...
		(pto unbparent (sref (ref left lft) (ref right X) (ref data d1) (ref balance b1)))
		(pto unbalance (sref (ref left V) (ref right Z) (ref data d2) (ref balance b5)))
		(pto lft (sref (ref left U) (ref right unbalance) (ref data d4) (ref balance b6)))
		(index alpha2 (ubavlhole U parent M9 H11 M7 H12))
		(index alpha3 (avl V M10 H10))
		(pto parent (sref (ref left x) (ref right nil) (ref data d3) (ref balance b8)))
		(pto x (sref (ref left nil) (ref right nil) (ref data key) (ref balance b9)))
		(index alpha4 (avl Z M5 H10))
		(index alpha5 (avl X M3 H3))
	))
	(= M4 (bagunion (bag d4) M9 M10))
	(< M9 (bag d4)) (< (bag d4) M10)
	(distinct b4 0) (= M7 (bagunion (bag d3) (bag key)) ) (< M4 (bag d2)) (< (bag d2) M5)
	(= M2 (bagunion (bag d1) (bag d2) M4 M5 M3) )
	(< (bagunion (bag d2) M4 M5) (bag d1)) (<  (bag d1) M3)
	(= b1 (- H3 (+ H10 2)) ) (<= (- 0 1) b1) (<= b1 1) (= M1 (bagunion M0 (bag key)) ) (= M0 (bagminus M0 (bag key)) ) 
	(distinct parent nil) (distinct unbparent nil) (> d1 key) (> d2 key) (> d3 key) (= cur x) (= ret  root) 
	(= b5 0) (= b6 0) (= H12 2) (= b8 (- 0 1) ) (= b9 0) (= H11 (+ H10 1) ) 
	)
)
