...
;; VC07: avlhole(root, unbparent1, M1,H1,M2,H2) * unbparent1 |-> ((left,unbalance1),(right,X),(data,d1),(balance,b1)) * 
;; parent1 |-> ((left,cur1),(right,Y),(data,d2),(balance,b2)) * cur1 |-> ((left,V),(right,U),(data,d3),(balance,b3)) * 
;; avl(V, M7, H7) * avl(U,M8,H8) * avl(Y, M5,H5) * avl(X, M3, H3) & M4 = {d3} cup M7 cup M8 & M7 < d3 < M8 & 
;; ite(H8>H7,H4 = H8+1, H4 = H7+1) & b2 = H8 - H7 & -1 <= b2 <= 1 & M4 < d2 < M5 & b2 = H5 - H4 & -1 <= b2 <= 1 & 
;; ite(key in M4, M2 = {d1} cup {d2} cup M4 cup M5 cup M3, M2 = {d1} cup {d2} cup M4 cup M5 cup M3 cup {key}) & 
;; ite(H5 > H4, h = H5+1, h = H4+1) & ite(H3 > h, H2 = H3+1, H2 = h+1) & {d2} cup M4 cup M5 < d1 < M3 & b1 = H3 - h & 
;; -1<= b1 <= 1 & ite(key in M0, M1 = M0, M1 = M0 cup {key}) & key in M0 <=> key in M4 & ! parent1 = nil & ! unbparent1 = nil & 
;; unbalance1 = parent1 & d1> key & d2 > key & ! b2 = 0 & ! cur1 = nil & b3 =0 & parent2 = cur1 & d3 > key & cur2 = V & 
;; unbparent2 = unbparent1 & unbalance2 = unbalance1 & ite(key in M7, M6 = M4, M6 = M4 cup {key}) |-
;; avlhole(root, unbparent2, M1,H1,M2,H2) * unbparent2 |-> ((left,unbalance2),(right,X),(data,d1),(balance,b1)) * 
;; unbalance2 |-> ((left,parent2),(right,Y),(data,d2),(balance,b2)) * bavlhole(parent2,parent2,M6,H4,M6,H4) * parent2 |-> ((left,cur2),(right,U),(data,d3),(balance,b3)) * avl(cur2, M7, H7) * avl(U,M8,H8) * avl(Y, M5,H5) * avl(X, M3, H3) & 
;; M7 < d3 < M8 & b3 = H8 - H7 & -1 <= b3 <= 1 & ite(key in M7, M6 = {d3} cup M7 cup M8, M6 = {d3} cup M7 cup M8 cup {key}) & 
;; ite(H8>H7, H4 = H8+1, H4 = H7+1) & M6 < d2 < M5 & b2 = H5 - H4 & -1 <= b2 <= 1 & M2 = {d1} cup {d2} cup M6 cup M5 cup M3 & 
;; {d2} cup M6 cup M5 < d1 < M3 & ite(H5>H4, h = H5 +1, h = H4 +1) & ite(H3 > h, H2 = H3+1, H2 = h+1) & b1 = H3 - h  & 
;; -1<= b1 <= 1 & ite(key in M0, M1 = M0, M1 = M0 cup {key}) & key in M0 <=> key in M7 & ! parent2 = nil & ! unbparent2 = nil & 
;; d1> key & d2 > key & d3 > key & b3 = 0 
...
(assert 
	...
	(ssep 
		...
		(pto cur1 (sref (ref left V) (ref right U) (ref data d3) (ref balance b3)) )	
		(index alpha2 (avl V M7 H7)) 
		(index alpha3 (avl U M8 H8))
		(index alpha4 (avl Y M5 H5))
		(index alpha5 (avl X M3 H3))
	))
	(= M4 (bagunion (bag d3) M7 M8) ) (< M7 (bag d3)) (< (bag d3) M8) 
	(ite (> H8 H7) (= H4 (+ H8 1)) (= H4 (+ H7 1)) )
	(= b2 (- H8 H7))
	(<= -1 b2) (<= b2 1)
	(< M4 (bag d2)) (< (bag d2 M5) (= b2 (- H5 H4) ) (<= -1 b2) (<= b2 1)
	(ite (subset (bag key) M4) (= M2 (bagunion (bag d1) (bag d2) M4 M5 M3) ) 
		(= M2 (bagunion (bag d1) (bag d2) M4 M5 M3 (bag key))) )
	(ite (> H5 H4) (= h (+ H5 1)) (= h  (+ H4 1)) ) 
	(ite (> H3 h) (= H2 (+ H3 1)) (= H2 (+ h 1)) ) 
	(< (bagunion (bag d2)  M4  M5) (bag d1))(< (bag d1) M3) (= b1 (- H3 h)) (<= -1 b1) (<= b1 1)
	(ite (subset (bag key) M0) (= M1 M0) (= M1  (bagunion M0 (bag key))) )
	(iff (subset (bag key) M0) (subset (bag key) M4) ) (distinct parent1 nil)
	(distinct unbparent1 nil) (= unbalance1 parent1) (> d1 key) (> d2 key) 
	(distinct b2 0) (distinct cur1 nil) (= b3 0)  (= parent2 cur1) (> d3 key) (= cur2 V)
	(= unbparent2 unbparent1) (= unbalance2 unbalance1) (ite (subset (bag key) M7) (= M6  M4) (= M6 (bagunion M4 (bag key)) ) )
	)
)
...
(assert (not 
	...
	(tobool 
	(ssep 
		(index alpha6 (avlhole root unbparent2 M1 H1 M2 H2)) 
		(pto unbparent2 (sref (ref left unbalance2) (ref right X) (ref data d1) (ref balance b1)))
		(pto unbalance2 (sref (ref left parent2) (ref right Y) (ref data d2) (ref balance b2)))
		(index alpha7 (bavlhole parent2 parent2 M6 H4 M6 H4))
		(pto parent2 (sref (ref left cur2) (ref right U) (ref data d3) (ref balance b3)) ) 
		(index alpha8 (avl cur2 M7 H7)) 
		(index alpha9 (avl U M8 H8)) 
		(index alpha10 (avl Y M5 H5)) 
		(index alpha11 (avl X M3 H3))
	))
	(< M7 (bag d3)) (< (bag d3) M8) (= b3  (- H8 H7)) (<= -1 b3) (<= b3 1) 
	(ite (subset (bag key) M7) (= M6 (bagunion (bag d3) M7 M8)) (= M6 (bagunion (bag d3) M7 M8 (bag key)) ) ) & 
	(ite (> H8 H7) (= H4 (+ H8 1)) (= H4 (+ H7 1)) ) (< M6 (bag d2))(< (bag d2) M5)
	(= b2 (- H5 H4)) (<= -1 b2) (<= b2 1)
	(= M2 (bagunion (bag d1) (bag d2) M6 M5 M3) )
	(< (bagunion (bag d2) M6 M5) (bag d1)) (< (bag d1) M3)
	(ite (> H5 H4) (= h (+ H5 1)) (= h (+ H4 1)) ) 
	(ite (> H3 h) (= H2 (+ H3 1)) (= H2 (+ h 1)) (= b1 (- H3 h))
	(<= -1 b1) (<= b1 1) (ite (subset (bag key) M0) (= M1 M0) (= M1 (bagunion M0 (bag key)) ) ) 
	(iff (subset (bag key) M0) (subset (bag key) M7) ) (distinct parent2 nil)
	(distinct unbparent2 nil) (> d1 key) (> d2 key) (> d3 key) (= b3 0) 
	)
))
