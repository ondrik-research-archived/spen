...
;; !cur1 = nil & parent2 = cur1 & d2 > key & cur2 = V & M3 = M4 & H3 = H4 & ite(key in M1, M4 = M0, M4 = M0 cup {key})
;; |-
...
(assert 
	...
	(tobool 
	(ssep 
		(pto root (sref (ref left cur1) (ref right Y) (data d1) (balance b1) ) ) 
		(pto cur1 (sref (ref left V) (ref right U) (data d2) (balance b2) ) ) 
	...
	(iff (subset (bag key) M0) (subset (bag key) M1) )
	(= parent1 root)
	(= unbparent1 nil)
	(= unbalance1 root) 
	(> d1 key) (distinct b2  0) (= unbalance2 cur1) (= unbparent2  parent1)
	(distinct cur1  nil) (= parent2  cur1) (> d2  key) (= cur2 V) (= M3 M4) (= H3 H4)
	ite( (subset (bag key)  M1) (= M4  M0) (= M4 (bagunion M0 (bag key) ))
	)
)
...
(assert (not 
	...
	(< M6 (bag d2)) (< (bag d2) M7)
	(= b2 (- H7 H6)) (<= -1 b2) (<= b2 1) 
	(ite (subset (bag key) M6) (= M4  (bagunion (bag d1)  (bag d2) M6  M7 M2) ) 
	(= M4 (bagunion (bag d1) (bag d2) M6  M7 M2 (bag key)) ) )
	(ite (> H7 H6) (= H1 (+ H7 1)) (= H1 (+ H6 1)) ) 
	(ite (> H2  H1) (= H4  (+ H2 1)) (= H4 (+ H1 1) ) 
	(< (bagunion (bag d2)  M6  M7) (bag d1)) (< (bag d1) M2) 
	(= b1  (- H2 H1) (<= -1 b1) (<= b1 1)
	( ite (subset (bag key) M0) (= M3  M0) (= M3  (bagunion M0 (bag key))) )
	(iff (subset (bag key) M0) (subset (bag key) M6)) 
