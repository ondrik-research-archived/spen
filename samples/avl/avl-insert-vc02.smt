...
;; avl(V,M8,H8) * avl(U,M9,H9) * avl(Z, M5, H5) * avl(Y, M3, H3) & M4 = {d3} cup M8 cup M9 & M8 < d3 < M9 & 
;; ite(H9>H8, H4 = H9 +1, H4 = H8 +1) & b3 = H9-H8 & -1<=b3<=1 & M4 < d2 < M5 & b2 = H5 - H4 & -1 <= b2 <= 1 & 
;; ite(key in M4, M2 = {d2} cup M4 cup M5, M2 = {d2} cup M4 cup M5 cup {key}) & ite(H5>H4, H2 = H5+1, H2 = H4+1) & 
;; ite(key in M0, M0 = {d1} cup M1 cup M3, M0 = ({d1} cup M1 cup M3) \ {key}) & M1 < d1 < M3 & b1 = H3 - H1 & -1<= b1 <= 1 & 
;; key in M0 <=> key in M4 & ! parent1 = nil & unbparent1 = nil & unbalance1 = root & d1 > key & d2 > key & ! b3 =0 & 
;; unbalance2 = cur1 & unbparent2 = parent1 & !cur1 = nil & parent2 = cur1 & d3 > key & cur2 = V & M6 = {d1} cup M1 cup M3 & 
;; M7 = M2 & H7 = H2 |-
;; avlhole(root, unbparent2, M6,H6,M7,H7) * unbparent2 |-> ((left,unbalance2),(right,Z),(data,d2),(balance,b2)) * 
;; parent2 |-> ((left,cur2),(right,U),(data,d3),(balance,b3)) * avl(cur2, M8, H8) * avl(U,M9,H9) * avl(Z, M5,H5) & M8 < d3 < M9 & 
;; b3 = H9 - H8 & -1 <= b3 <= 1 & ite(key in M8, M7 = {d2} cup {d3} cup M8 cup M9 cup M5, M7 = 
;; {d2} cup {d3} cup M8 cup M9 cup M5 cup {key}) & ite(H9 > H8, H4=H9+1, H4=H8+1) & ite(H5 > H4, H7 = H5+1, H7 = H4+1) & 
;; {d3} cup M8 cup M9 < d2 < M5 & b2 = H5 - H4 & -1<= b2 <= 1 & ite(key in M0, M6 = M0, M6 = M0 cup {key}) & 
;; key in M0 <=> key in M8 & ! parent2 = nil & ! unbparent2 = nil & unbalance2 = parent2 & d2> key & d3 > key & ! b3 = 0 
...
(assert 
	...
	(index alpha1 (bavlhole X parent1 M1 H1 M2 H2))
 	(pto parent1 (sref (ref left cur1) (ref right Z) (ref data d2) (ref balance b2)) ) 
	(pto cur1  (sref (ref left V) (ref right U) (ref data d3) (ref balance b3) ) )
	(index alpha2 (avl V M8 H8)) 
	(index alpha3 (avl U M9 H9) 
	(index alpha4 (avl Z M5 H5))
	(index alpha5 (avl Y M3 H3))
	))
	(= M4 (bagunion (bag d3) M8 M9))(< M8 (bag d3) (< (bag d3) M9)
	( ite (> H9 H8) (= H4 (+ H9 1) (= H4 (+ H8 1)) ) (= b3 (- H9 H8) 
	(<= -1 b3)(<= b3 1) (< M4 d2) (< d2 M5)
	(= b2 (- H5 H4)) (<= -1 b2) (<= b2 1)
	(ite (subset (bag key) M4) (= M2 (bagunion (bag d2)  M4 M5 )) (= M2 (bagunion (bag d2) M4 M5 (bag key)) ) ) 
	(ite (> H5 H4) (= H2 (+ H5 1)) (= H2 (+ H4 1)) )
	(ite (subset (bag key) M0) (= M0 (bagunion (bag d1) M1 M3)) (= M0 (bagminus (bagunion (bag d1) M1 M3) (bag key)) ) )
	(< M1 (bag d1)) (< (bag d1) M3) (= b1 (- H3 H1)) (<= -1 b1)(<= b1 1)
	(iff (subset (bag key) M0) (subset (bag key) M4) ) 
	(distinct parent1 nil)  (= unbparent1 nil) (= unbalance1  root) (> d1 key) (> d2 key)
	(distinct b3 0) (= unbalance2 cur1) (= unbparent2 parent1) (distinct cur1 nil)
	(= parent2 cur1) (> d3 key) (= cur2 V) (= M6  (bagunion (bag d1)  M1  M3) ) 
	(= M7 M2) (= H7 H2)
	)
)
...
(assert (not 
	...
	(tobool 
	(ssep 
	(index alpha6 (avlhole root unbparent2 M6 H6 M7 H7))
	(pto unbparent2 (sref (ref left unbalance2) (ref right Z) (ref data d2) (ref balance b2) )	)
	(pto parent2 (sref (ref left cur2) (ref right U) (ref data d3) (ref balance b3)) )
	(index alpha7 (avl cur2 M8 H8))
	(index alpha8 (avl U M9 H9))
	(index alpha9 (avl Z M5 H5))
	))
	(< M8 (bag d3)) ((bag d3) M9)
	(= b3  (- H9 H8)) (<= -1 b3) (<= b3 1) 
	(ite (subset (bag key) M8) (= M7 (bagunion (bag d2) (bag d3) M8 M9 M5) ) (= M7
	(bagunion (bag d2) (bag d3) M8 M9 M5 (bag key)) )
	(ite (> H9 H8) (= H4 (+ H9 1)) (= H4 (+ H8 1)) ) 
	(ite (> H5 H4) (= H7  (+ H5 1)) (= H7  (+ H4 1)) )	
	(< (bagunion (bag d3) M8 M9) (bag d2)) (< (bag d2) M5) 
	(= b2 (- H5 H4)) (<= -1 b2) (<= b2 1) 
	(ite (subset (bag key) M0) (= M6  M0) (= M6  (bagunion M0 (bag key)) )
	(iff (subset (bag key) M0) (subset (bag key) M8) ) (distinct  parent2  nil)
	(distinct unbparent2  nil) (= unbalance2 parent2) (> d2 key) (> d3 key)(distinct b3 0) 
	)
))
