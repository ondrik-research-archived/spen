...
;; VC02: root0|->((left,X), (right,Y), (data, d)) * bst(X, M1) * bst(Y, M2) & M0 = {d} cup M1 cup M2 & M1 < d < M2 & d > key & 
;; cur1 = root & parent1 = nil & ! root0 = nil & parent2 = cur1 & cur2 = X |-
;; root0|->((left,cur2), (right,Y), (data, d)) * bst(cur2, M1) * bst(Y, M2) & M0 = {d} cup M1 cup M2 & M1 < d < M2 & cur1 = root0 &
;; parent2 = cur1 & key in M0 <=> key in M1 & ! parent2 = nil
...
(assert (not 
	...
	(tobool 
	(ssep 
		(pto root0 (sref (ref left cur2) (ref right Y) (data d) ) ) 
		(index alpha3 (bst cur2 M1) )
		(index alpha4 (bst Y M2) )
	))
	(= M0 (bag-union (singleton d) (bag-union M1 M2)) )
	(< M1 (singleton d) )
	(< (singleton d) M2)
	(= cur1 root0)
	(= parent2 cur1)
	(iff (bag-sub (singleton key) M0) (bag-sub (singleton key) M1) )
