...
;; VC2: bsthole(root, parent1, M1, M2) * parent1 |-> ((left,X), (right,cur1)) * bst(X,M3) * cur1 |-> ((left,cur2), (right,Z)) * 
;; bst(cur2, M5) * bst(Z, M6) & M1 = M0 \ {key} & M2 = ({parent1.data} cup M3 cup M4) \ {key} & M3 < parent1.data < M4 & 
;; M4 = {cur1.data} cup M5 cup M6 & M5 < cur1.data < M6 &  parent1.data < key & cur1.data > key & parent2 = cur1 & 
;; (key in M0 <=> key in M4) & M22 = M4 \ {key} |-
;; bsthole(root,parent2, M1, M22) * parent2 |-> ((left,cur2), (right,Z)) * bst(cur2, M5) * bst(Z, M6) & M1 = M0 \ {key} & 
;; M22 = ({parent2.data} cup M5 cup M6) \ {key}  & M5 < parent2.data < M6  & key in M0 <=> key in M5 & parent2.data > key
...
(assert 
	...
	(ssep 
		...
		(pto cur1 (sref (ref left cur2) (ref right Z) (data d2) ) ) 
		(index alpha3 (bst cur2 M5) )
...
(assert (not 
	...
	(ssep 
		...
		(index alpha6 (bst cur2 M5) )
		(index alpha7 (bst Z M6) )
	))
	(= M1 (bag-diff M0 (singleton key)) )
	(= M22 (bag-diff (bag-union (singleton d2) (bag-union M5 M6)) (singleton key)) )
	(bag-lt M5 (singleton d2) )
	(bag-lt (singleton d) M6)
	(or (not (bag-sub (singleton key) M0)) (bag-sub (singleton key) M5))
	(or (not (bag-sub (singleton key) M5)) (bag-sub (singleton key) M0))
	(> d2 key)
	)
))
