...
;; VC13: bsthole(root0, parent, M1, M2) * parent |-> ((left,X), (right,cur), (data, d1)) * bst(X, M3) * bst(Y, M6) * 
;; Z |-> ((left, Y), (right, V), (data, d3)) * bst(U, M8) * bst(V, M9) & M7 = {d3} cup M8 cup M9 & M8 < d3 < M9 & 
;; M4 = {d2} cup M6 cup M7 & M6 < d2 < M7 & M1 = M0 \ {key} & M2 = ({d1} cup M3 cup M4) \ {key} & M3 < d1 < M4 & !(parent = nil) &
;; d1 < key & (key in M0 <=> key in M4) & d2 = key & keynode = cur &  !Z = nil & rgt = Z & 
;; U = nil & subroot = rgt & M5 = {d3} cup M6 cup M9 |-
...
(assert 
	...
	(ssep 
		...
		(pto Z (sref (ref left Y) (ref right V) (ref data d3) ) ) 
		(index alpha4 (bst U M8) )
		(index alpha5 (bst V M9) )
	))
	(= M7 (bag-union (singleton d3) (bag-union M8 M9) ) )
	(< M8 (singleton d3) )
	(< (singleton d3) M9)
	...
	(= U nil) 
	(= subroot rgt)
 	(= M5 (bag-union (singleton d3) (bag-union M6 M9)) )
	)
)
