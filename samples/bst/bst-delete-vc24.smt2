...
;; VC24: bsthole(root0, parent, M1, M2) * parent |-> ((left, subroot), (right,Y), (data, d1)) * bst(subroot, M5) * bst(Y,M4) & 
;; M1 = M0 \ {key} & M5 = M3 \ {key} & M2 = {d1} cup M5 cup M4 & M5 < d1 < M4 & d1 > key & key in M3 & key in M0 & 
;; !(parent = nil) & ret = root0 |-
...
(assert 
	...
	(ssep
		...
		(pto parent (sref (ref left subroot) (ref right Y) (ref data d1)))
		(index alpha2 (bst subroot M5) )
		(index alpha3 (bst Y M4))
	))
	(= M1 (bag-diff M0 (singleton key)))
	(= M5 (bag-diff M3 (singleton key)))	
	(= M2 (bag-union (singleton d1) (bag-cup M5 M4) ) )
	(< M5 (singleton d1) )
	(< (singleton d1) M4)
	(> d1 key)
