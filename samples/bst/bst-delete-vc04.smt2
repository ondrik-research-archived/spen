...
;; VC04: bsthole(root0, parent1, M1, M2) * parent1 |-> ((left,cur1), (right,Y), (data, d1)) * cur1 |-> ((left,X), (right,Z), (data, d2)) * 
;; bst(X, M6) * bst(Z, M7) * bst(Y,M4) & M1 = M0 \ {key} & M2 = ({d1} cup M3 cup M4) \ {key} & M3 < d1 < M4 & M3 = {d2} cup M6 cup M7 & 
;; M6 < d2 < M7 & ! parent1 =nil & d1 > key & (key in M0 <=> key in M3) & ! cur1 = nil & d2 > key & parent2 = cur1 & cur2 = X & 
;; M5 = M3 \ {key} |-
;; bsthole(root0,parent2, M1, M5) * parent2 |-> ((left,cur2), (right,Z), (data, d2)) * bst(cur2, M6) * bst(Z, M7) & M1 = M0 \ {key} & 
;; M5 = ({d2} cup M6 cup M7) \ {key}  & M6 < d2 < M7 & !(parent2 = nil) & d2 > key & key in M0 <=> key in M6
...
(assert (not 
	...
	(> d2 key)
	(iff (bag-sub (singleton key) M0) (bag-sub (singleton key) M6) )
	)
))
