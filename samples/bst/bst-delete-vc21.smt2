...
(assert 
	...
	(= M11 (bag-union (singleton d3) (bag-union M14 M15))
	(< M5 (singleton d3) )
	...
	(= M2 (bag-diff (bag-union (singleton d1) (bag-union M3 M4)) (singleton keymin)) )
	(< M3 (singleton d1))
	(< (singleton d1) M4)
	(distinct parent nil)
	(> d1 key)
	...
	(= V nil)
	(= subroot cur)
	(= M13 (bag-union (singleton d3) (bag-union M5 M8)) )
	)
)
...
(assert (not 
	...
	(= M13 (bag-diff M3 (singleton key)) )
	(= M2 (bag-union (singleton d1) (bag-union M13 M14)) )
	(< M13 (singleton key))
	(< (singleton key) M4)
	(> d1 key)
