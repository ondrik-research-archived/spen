...
;; VC06: bsthole(root0, parent1, M1, M2) * parent1 |-> ((left,cur1), (right,Y), (data, d1)) * 
;; cur1 |-> ((left,Z), (right,X), (data, d2)) * bst(Z, M6) * bst(X, M7) * bst(Y,M4) & M1 = M0 \ {key} & 
;; M2 = ({d1} cup M3 cup M4) \ {key} & M3 < d1 < M4 & M3 = {d2} cup M6 cup M7 & M6 < d2 < M7 & ! parent1 =nil & d1 > key & 
;; (key in M0 <=> key in M3) & ! cur1 = nil & d2 < key & parent2 = cur1 & cur2 = X |-
...
(assert 
	...
	(ssep 
		...
		(pto cur1 (sref (ref left Z) (ref right X) (ref data d2) ) ) 
		(index alpha2 (bst Z M6)) 
		(index alpha3 (bst X M7) )
