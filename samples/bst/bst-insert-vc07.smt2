...
(assert (not 
	...
	(ssep 
		...
		(pto parent2 (sref (ref left X) (ref right cur2) (data d2) ) ) 
	...
	(ite (bag-sub (singleton key) M0) (= M1 M0) (= M1 (bag-union M0 (singleton key)) ) ) 
	(= M3 (bag-union (singleton d2) (bag-union M6 M7) ) )
	(ite (bag-sub (singleton key) M7) (= M5 M3) (= M5 (bag-union M3 (singleton key) )) ) 
	(or (not (bag-sub (singleton key) M0) ) (bag-sub (singleton key) M7) )
	(or (not (bag-sub (singleton key) M7) ) (bag-sub (singleton key) M0) )
	(< d2 key)
