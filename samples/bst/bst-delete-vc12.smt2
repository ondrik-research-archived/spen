...
;; VC12: bsthole(root0, parent, M1, M2) * parent |-> ((left,cur), (right,Y), (data, d1)) * bst(X, M6) * 
;; Z |-> ((left,X),(right,V), (data, d3)) * bst(U, M8) * bst(V, M9) * bst(Y,M4) & M7 = {d3} cup M8 cup M9 & M8 < d3 < M9 & 
;; M3 = {d2} cup M6 cup M7 & M6 < d2 < M7 & M1 = M0 \ {key} & M2 = ({d1} cup M3 cup M4) \ {key} & M3 < d1 < M4 & !(parent = nil) & 
;; d1 > key & (key in M0 <=> key in M3) & d2 = key & keynode = cur & !Z = nil & rgt = Z & U = nil & subroot = rgt & 
;; M5 = {d3} cup M6 cup M9 |-
...
(assert 
	...
	(ssep 
		...
		(pto Z (sref (ref left X) (ref right V) (ref data d3) ) ) 
...
(assert (not 
	...
	(ssep 
		...
		(index alpha7 (bst subroot M5) )
		(index alpha8 (bst Y M4) )
	))
	(= M1 (bag-diff M0 (singleton key) ) )
