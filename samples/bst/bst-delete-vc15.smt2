...
;; VC15: bsthole(root0, parent, M1, M2) * parent |-> ((left,cur1), (right,Y), (data, d1)) * cur1 |-> ((left,X), (right,U), (data, d2)) * 
;; bst(X, M5) * U |-> ((left,V), (right, Z), (data, d3)) * bst(V, M6) * bst(Z, M7) * bst(Y, M4) & M10 = {d3} cup M6 cup M7 & M6 < d3 < M7 & 
;; M3 = {d2} cup M5 cup M10 & M5 < d2 < M10 & M1 = M0 \ {key} & M2 = ({d1} cup M3 cup M4) \ {key} & M3 < d1 < M4 & !(parent = nil) & d1 > key & 
;; (key in M0 <=> key in M3) & keynode = cur1 & d2 = key & ! U = nil & rgt = U & ! V = nil & cur2 = V & nxtparent2 = rgt & M8 = M9 & 
;; M9 = M10 \ {keymin} & keymin in M6 & keymin <= M6 |-
;; bsthole(root0, parent, M1, M2) * parent |-> ((left, keynode), (right,Y), (data, d1)) * keynode |-> ((left,X), (right, rgt), (data, key)) * 
;; bst(X,M5) * bsthole(rgt, nxtparent2, M8, M9) * nxtparent2 |-> ((left, cur2), (right, Z), (data, d3)) * bst(cur2, M6) * bst(Z, M7) * bst(Y,M4) & 
;; M5 < key < M8 cup {keymin} & M9 = ({d3} cup M6 cup M7) \ {keymin} & M6 < d3 < M7 & M3 = {key} cup M5 cup M8 cup {keymin} & key in M0 & 
...
(assert 
	...
	(ssep 
		...
		(pto parent (sref (ref left cur1) (ref right Y) (ref data d1) ) ) 
		(pto cur1 (sref (ref left X) (ref right U) (ref data d2) ) ) 
		(index alpha2 (bst X M5) )
		(pto U (sref (ref left V) (ref right Z) (ref data d3) ) ) 
		(index alpha3 (bst V M6) )
		(index alpha4 (bst Z M7) )
		(index alpha5 (bst Y M4) )
	))
	(= M10 (bag-union (singleton d3) (bag-union M6 M7) ) )
	(< M6 (singleton d3) )
	(< (singleton d3) M7)
	(= M3 (bag-union (singleton d2) (bag-union M5 M10) ) )
	(< M5 (singleton d2) )
	(< (singleton d2) M10 )
...
(assert (not 
	...
	(ssep 
		...
		(index alpha7 (bst X M5) )
		(index alpha8 (bsthole rgt nxtparent2 M8 M9) )
		(pto nxtparent2 (sref (ref left cur2) (ref right Z) (ref data d3) ) ) 
		(index alpha9 (bst cur2 M6) )
		(index alpha10 (bst Z M7) )		
		(index alpha11 (bst Y M4) )		
	))
	(< M5 (singleton key) )
