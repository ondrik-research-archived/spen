...
;; VC19: bsthole(root0, parent, M1, M2) * parent |-> ((left, X), (right, keynode), (data, d1)) * 
;; keynode |-> ((left,Y), (right, rgt), (data, key)) * bst(X,M3) * bst(Y,M5) * bsthole(rgt, nxtparent1, M8, M9) * 
;; nxtparent1 |-> ((left, cur1), (right, Z), (data, d3)) * cur1 |-> ((left, V), (right, U), (data, d4)) * bst(V, M11) * 
;; bst(U, M12) * bst(Z, M7) & M6 = {d4} cup M11 cup M12 & M11 < d4 < M12 & M5 < key < M8 cup {keymin} & 
;; M9 = ({d3} cup M6 cup M7) \ {keymin} & M6 < d3 < M7 & M4 = {key} cup M5 cup M8 cup {keymin} & key in M0 & 
;; M1 = M0 \ {key} & M2 = ({d1} cup M3 cup M4) \ {key} &  M3 < d1 < M4 & !(parent = nil) & d1 < key & !cur1 = nil & 
;; nxtparent2 = cur1 &  ! V = nil & cur2 = V & keymin in M6 & keymin <= M6 & M10 = M6 \ {keymin} |-
;; bsthole(root0, parent, M1, M2) * parent |-> ((left, X), (right, keynode), (data, d1)) * 
;; keynode |-> ((left,Y), (right, rgt), (data, key)) * bst(X,M3) * bst(Y,M5) * bsthole(rgt, nxtparent2, M8, M10) * 
;; nxtparent2 |-> ((left, cur2), (right, U), (data, d4)) * bst(cur2, M11) * bst(U, M12) & M5 < key < M8 cup {keymin} & 
;; M10 = ({d4} cup M11 cup M12) \ {keymin} & M11 < d4 < M12 & M4 = {key} cup M5 cup M8 cup {keymin} & key in M0 & M1 = M0 \ {key} &
...
(assert 
	...
	(ssep 
		...
		(pto parent (sref (ref left X) (ref right keynode) (data d1) ) ) 
		(pto keynode (sref (ref left Y) (ref right rgt) (data key) ) ) 
		(index alpha2 (bst X M3) )
		(index alpha3 (bst Y M5) )
		(index alpha4 (bsthole rgt nxtparent1 M8 M9) )
		(pto nxtparent1 (sref (ref left cur1) (ref right Z) (data d3) ) ) 
		(pto cur1 (sref (ref left V) (ref right U) (data d4) ) ) 
		(index alpha5 (bst V M11) )
		(index alpha6 (bst U M12) )
		(index alpha7 (bst Z M7) )
	))
	(= M6 (bag-union (singleton d4) (bag-union M11 M12) ) )
	(< M11 (singleton d4) )
	(< (singleton d4) M12)
	(< M5 (singleton key) )
	(< (singleton d2) (bag-union M8 (singleton keymin)) )
	...
	(<= (singleton keymin) M6)
	(= M10 (bag-diff M6 (singleton keymin)))
	)
)
...
(assert (not 
	...
	(ssep 
		...
		(index alpha9 (bst X M3) )
		(index alpha10 (bst Y M5) )
		(index alpha11 (bsthole rgt nxtparent2 M8 M10) )
		(pto nxtparent2 (sref (ref left cur2) (ref right U) (ref data d4) ) ) 
		(index alpha12 (bst cur2 M11) )
		(index alpha13 (bst U M12) )
	))
	(< M5 (singleton key) )
	(< (singleton key) (bag-union M8 (singleton keymin)) )
	(= M10 (bag-diff (bag-union (singleton d4) (bag-union M11 M12) ) (singleton keymin) ) )
	(< M11 (singleton d4) )
	(< (singleton d4) M12)
	(= M3 (bag-union (singleton key) (bag-union M5 (bag-union M8 (singleton keymin)))))
	(bag-sub (singleton key) M0)
	(= M1 (bag-diff M0 (singleton key)))
	(= M2 (bag-diff (bag-union (singleton d1) (bag-union M3 M4) ) (singleton key) ) )
	(< M3 (singleton d1))
