...
(assert 
	(and (distinct x_emp w_emp) (distinct w_emp t_emp) (distinct y_emp w_emp) 
	(tobool 
	(ssep 
		(pto w_emp (sref (ref next t_emp) (ref prev u_emp) ) ) 
		(index alpha0 (dll x_emp u_emp y_emp w_emp )) 
		(index alpha1 (dll t_emp y_emp w_emp x_emp )) 
	)
...
(assert (not 
	(tobool 
		(index alpha2 (dll x_emp y_emp y_emp x_emp )) 
	)
