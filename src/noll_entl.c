...
int noll_entl_solve(void) {
  ...
  noll_entl_type();
	...
	NOLL_DEBUG("Concrete graph nodes = %u\n", noll_prob->pgraph->nodes_size);
	noll_ta_t* graph_ta = noll_graph2ta(noll_prob->pgraph);
	assert(NULL != graph_ta);

	NOLL_DEBUG("\nGraph TA:\n");
	vata_print_ta(graph_ta);
	NOLL_DEBUG("\n");
	...
	assert(NULL != pred_graph->svars);
	assert(NULL != pred_graph->var2node);
	assert(NULL != pred_graph->edges);

	NOLL_DEBUG("pred_graph = %p\n", pred_graph);
	NOLL_DEBUG("number of nodes in pred_graph = %d\n", pred_graph->nodes_size);
	NOLL_DEBUG("LVars:\n");
	for (size_t i = 0; i < noll_vector_size(pred_graph->lvars); ++i)
	{
		NOLL_DEBUG("  (*pred_graph->lvars)[%lu] = %p, ", i, noll_vector_at(pred_graph->lvars, i));
		const noll_var_t* var = noll_vector_at(pred_graph->lvars, i);
		assert(NULL != var);
		NOLL_DEBUG("name = %s, vid = %u, ", var->vname, var->vid);
		NOLL_DEBUG("points to node -> %u\n", pred_graph->var2node[i]);
	}
	NOLL_DEBUG("SVars:\n");
	for (size_t i = 0; i < noll_vector_size(pred_graph->svars); ++i)
	{
		NOLL_DEBUG("  (*pred_graph->svars)[%lu] = %p, ", i, noll_vector_at(pred_graph->svars, i));
		const noll_var_t* var = noll_vector_at(pred_graph->svars, i);
		assert(NULL != var);
		NOLL_DEBUG("name = %s, vid = %u, \n", var->vname, var->vid);
	}
	NOLL_DEBUG("Edges:\n");
	for (size_t i = 0; i < noll_vector_size(pred_graph->edges); ++i)
	{
		NOLL_DEBUG("  (*pred_graph->edges)[%lu] = %p, ", i, noll_vector_at(pred_graph->edges, i));
		const noll_edge_t* edge = noll_vector_at(pred_graph->edges, i);
		assert(NULL != edge);
	...
	assert(3 == pred_graph->nodes_size);
	assert(1 == noll_vector_size(pred_graph->edges));
	...
	assert(NULL != pred_ta);
	NOLL_DEBUG("\nPredicate TA:\n");
	vata_print_ta(pred_ta);

	bool inclRes = vata_check_inclusion(graph_ta, pred_ta);
	vata_free_ta(graph_ta);
	vata_free_ta(pred_ta);
