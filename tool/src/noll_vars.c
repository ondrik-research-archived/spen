...
void noll_var_register(noll_var_array* a, const char* name, noll_type_t* ty,
		noll_scope_e scope) {
...
uid_t noll_var_array_find_local(noll_var_array* a, const char* name) {
...
uid_t noll_var_record(noll_var_array* a, uid_t vid) {
	assert (a);
	if (vid >= noll_vector_size (a)) {
