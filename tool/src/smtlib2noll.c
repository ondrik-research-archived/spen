...
static void smtlib2_noll_parser_declare_function(smtlib2_parser_interface *p,
		const char *name, smtlib2_sort sort);
static void smtlib2_noll_parser_define_function(smtlib2_parser_interface *p,
		const char *name, smtlib2_vector *params, smtlib2_sort sort,
		smtlib2_term term);
...
static smtlib2_sort smtlib2_noll_parser_make_sort(smtlib2_parser_interface *p,
		const char *sortname, smtlib2_vector *index);
static smtlib2_sort smtlib2_noll_parser_make_parametric_sort(
		smtlib2_parser_interface *p, const char *sortname, smtlib2_vector *tps);
static smtlib2_sort smtlib2_noll_parser_make_function_sort(
		smtlib2_parser_interface *p, smtlib2_vector *tps);

/* Terms */

static void smtlib2_noll_parser_declare_variable(
		smtlib2_parser_interface *parser, const char *name, smtlib2_sort sort);
static smtlib2_term smtlib2_noll_parser_push_quantifier_scope(
		smtlib2_parser_interface *p);
static smtlib2_term smtlib2_noll_parser_pop_quantifier_scope(
		smtlib2_parser_interface *p);
static smtlib2_term smtlib2_noll_parser_make_forall_term(
		smtlib2_parser_interface *p, smtlib2_term term);
static smtlib2_term smtlib2_noll_parser_make_exists_term(
		smtlib2_parser_interface *p, smtlib2_term term);
static smtlib2_term smtlib2_noll_parser_mk_function(smtlib2_context ctx,
		const char *symbol, smtlib2_sort sort, smtlib2_vector *index,
		smtlib2_vector *args);
...
smtlib2_noll_parser_new(void) {
	smtlib2_noll_parser *ret = (smtlib2_noll_parser *) malloc(
			sizeof(smtlib2_noll_parser));
	...
	smtlib2_abstract_parser_init((smtlib2_abstract_parser *) ret,
			(smtlib2_context) ret);
	ret->sorts_ = smtlib2_hashtable_new(smtlib2_hashfun_str, smtlib2_eqfun_str);
...
static void smtlib2_noll_parser_set_logic(smtlib2_parser_interface *p,
	...
	if (ap->response_ != SMTLIB2_RESPONSE_ERROR) {
		/* check that the logic is supported, i.e., QF_NOLL */
		...
		smtlib2_hashtable_set(noll_sorts (p), (intptr_t) smtlib2_strdup(
				"SetLoc"), (intptr_t) ty);
	}
}
...
static void smtlib2_noll_parser_declare_sort(smtlib2_parser_interface *p,
	...
	if (ap->response_ != SMTLIB2_RESPONSE_ERROR) {
		...
		if (smtlib2_hashtable_find (noll_sorts (p), (intptr_t) sortname, &k)) {
			...
			ap->response_ = SMTLIB2_RESPONSE_ERROR;
			ap->errmsg_ = smtlib2_sprintf(
					"sort declaration of arity != 0 not supported");
		} else {
			/* register the record */
			...
			if (ty != NULL) {
				...
				smtlib2_hashtable_set(noll_sorts (p),
						(intptr_t) smtlib2_strdup(sortname), (intptr_t) ty);
			} else {
				ap->response_ = SMTLIB2_RESPONSE_ERROR;
				ap->errmsg_ = smtlib2_sprintf(
						"sort declaration `%s' not supported", sortname);
			}
		}
	}
}
...
static void smtlib2_noll_parser_declare_function(smtlib2_parser_interface *p,
	...
	if (ap->response_ != SMTLIB2_RESPONSE_ERROR) {
		...
		if (smtlib2_hashtable_find (noll_funs (p), (intptr_t) name, &k)) {
			...
			noll_type_t* ty = noll_mk_fun_decl(noll_ctx (p), name, (noll_type_t*) sort);
			if (ty != NULL) {
				ap->response_ = SMTLIB2_RESPONSE_SUCCESS;
				smtlib2_hashtable_set(sp->funs_,
						(intptr_t) smtlib2_strdup(name), (intptr_t) ty);
			} else {
				ap->response_ = SMTLIB2_RESPONSE_ERROR;
				ap->errmsg_ = smtlib2_sprintf(
						"function declaration `%s' not supported", name);
			}
		}
	}
}
 ...
 */
static void smtlib2_noll_parser_declare_variable(smtlib2_parser_interface *p,
		const char *name, smtlib2_sort sort) {
	smtlib2_abstract_parser *ap = (smtlib2_abstract_parser *) p;
	...
	if (ap->response_ != SMTLIB2_RESPONSE_ERROR) {
		...
		if (ty != NULL) {
			...
			if (name[0] != '?') {
				ap->response_ = SMTLIB2_RESPONSE_ERROR;
				ap->errmsg_ = smtlib2_sprintf(
						"local variable `%s' is not a constant.", name);
...
static void smtlib2_noll_parser_define_function(smtlib2_parser_interface *p,
	...
	if (ap->response_ != SMTLIB2_RESPONSE_ERROR) {
		...
		if (smtlib2_hashtable_find (noll_funs (p), (intptr_t) name, &k)) {
			ap->response_ = SMTLIB2_RESPONSE_ERROR;
			ap->errmsg_ = smtlib2_sprintf(
					"predicate or function `%s' already declared", name);
		} else {
			...
			if (smtlib2_term_parser_error(ap->termparser_)) {
				ap->response_ = SMTLIB2_RESPONSE_ERROR;
				ap->errmsg_ = smtlib2_strdup(smtlib2_term_parser_get_error_msg(
						ap->termparser_));
			} else {
				...
				size_t pid = noll_mk_fun_def(ctx, name,
						smtlib2_vector_size (params), (noll_type_t*) sort,
						(noll_exp_t*) term);
				if (pid == UNDEFINED_ID) {
					ap->response_ = SMTLIB2_RESPONSE_ERROR;
					ap->errmsg_ = smtlib2_sprintf(
...
static smtlib2_sort smtlib2_noll_parser_make_function_sort(
	...
	if (ap->response_ != SMTLIB2_RESPONSE_ERROR) {
		...
		if (smtlib2_vector_size (tps) > 1) {
			ap->response_ = SMTLIB2_RESPONSE_ERROR;
			ap->errmsg_ = smtlib2_sprintf(
					"make function with arity > 0 not supported");
		}
		// return the vector
		...
		if (!smtlib2_hashtable_find (noll_sorts (p), (intptr_t) sortname, &k)) {
			...
			if (!strcmp(sortname, "Field") && tps && smtlib2_vector_size (tps)
				...
				if (src->kind == NOLL_TYP_RECORD && dst->kind
						== NOLL_TYP_RECORD)
					res = (smtlib2_sort) noll_mk_type_field(
							noll_vector_at(src->args,0),
				...
				ap->response_ = SMTLIB2_RESPONSE_ERROR;
				ap->errmsg_ = smtlib2_sprintf(
						"unsupported parametric sort `%s'", sortname);
			}
		}
	}
	return res;
